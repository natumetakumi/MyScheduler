...
import java.util.*
import io.realm.kotlin.where as where1
...
class ScheduleEditActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (scheduleId != -1L) {
            val schedule = realm.where1<Schedule>()
            ...
            when (scheduleId) {
                -1L -> {
                    ...
                    realm.executeTransaction { db: Realm ->
                        val maxId = db.where1<Schedule>().max("id")
                    ...
                    realm.executeTransaction { db: Realm ->
                        val schedule = db.where1<Schedule>()
                            .equalTo("id", scheduleId).findFirst()
        ...
        delete.setOnClickListener { view: View ->
            realm.executeTransaction { db: Realm ->
                db.where1<Schedule>().equalTo("id",scheduleId)
                    ?.findFirst()
                    ?.deleteFromRealm()
            }
